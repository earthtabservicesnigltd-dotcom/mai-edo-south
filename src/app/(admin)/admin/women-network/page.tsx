...
export default function AdminWomenNetworkPage() {
  const [members, setMembers] = useState<WomenMember[]>([])
  const [filtered, setFiltered] = useState<WomenMember[]>([])
  ...
  function exportToExcel() {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Women Network')
    XLSX.writeFile(workbook, `MAI-Women-Network-${new Date().toISOString().split('T')[0]}.xlsx`)
  }

  useEffect(() => {
    async function fetchMembers() {
      const res = await fetch('/api/admin/women-network')
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="font-heading text-3xl md:text-4xl text-[#01381d]">WOMEN NETWORK</h1>
          <p className="text-ink-muted text-sm mt-1">Manage registered women network members</p>
        </div>
        ...
        <CardHeader>
          <div className="flex flex-col gap-4 justify-between">
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
              <button onClick={approveAll} className="px-4 py-2 bg-green-600 text-white text-xs font-bold rounded-xl hover:bg-green-700 transition-colors uppercase tracking-wider whitespace-nowrap">✅ Approve All Pending</button>
              <Input placeholder="Search by name, email, ID..." value={search} onChange={e => setSearch(e.target.value)} className="field sm:w-72" />
              <button onClick={exportToExcel} className="px-4 py-2 bg-[#01381d] text-white text-xs font-bold rounded-xl hover:bg-[#015b2d] transition-colors uppercase tracking-wider whitespace-nowrap">📊 Export to Excel</button>
            </div>
          </div>
          <div className="flex gap-2 mt-4 flex-wrap">
            {STATUS_TABS.map(tab => (
              <button key={tab} onClick={() => setStatusFilter(tab)} className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider transition-colors ${statusFilter === tab ? 'bg-[#01381d] text-white' : 'bg-gray-100 text-ink-muted hover:bg-gray-200'}`}>{tab}</button>
            ))}
          </div>
        </CardHeader>
                ...
                <tbody>
                  {filtered.map(m => (
                    <tr key={m.id} className="border-b border-border last:border-0 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="py-3 px-2 font-semibold whitespace-nowrap"><Link href={`/admin/women-network/${m.id}`} className="hover:text-[#f97316] transition-colors">{m.full_name}</Link></td>
                      ...
                      <td className="py-3 px-2"><span className={`inline-block px-2 py-0.5 rounded-full text-xs font-bold ${m.status === 'approved' ? 'bg-green-100 text-green-700' : m.status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700'}`}>{m.status}</span></td>
                      <td className="py-3 px-2">
                        <div className="flex gap-2">
                          {m.status !== 'approved' && <button onClick={() => updateStatus(m.id, 'approved')} className="px-2 py-1 bg-green-100 text-green-700 text-xs font-bold rounded-lg hover:bg-green-200 transition-colors">Approve</button>}
                          {m.status !== 'rejected' && <button onClick={() => updateStatus(m.id, 'rejected')} className="px-2 py-1 bg-red-100 text-red-700 text-xs font-bold rounded-lg hover:bg-red-200 transition-colors">Reject</button>}
                        </div>
                      </td>
                    </tr>
                  ))}
