...
export default function BroadcastPage() {
  const [targetType, setTargetType] = useState<'students' | 'subscribers'>('students')
  ...
  async function handleSend() {
    ...
    const res = await fetch('/api/admin/broadcast', {
      ...
      body: JSON.stringify({ targetType, schoolSlug, subject, message, sender }),
    })
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-ink-muted text-sm mt-1">Send emails to Academy Students or Website Subscribers.</p>
        </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-bold text-ink-muted uppercase mb-1">Target Audience</label>
            <select value={targetType} onChange={e => setTargetType(e.target.value as 'students' | 'subscribers')} className="w-full px-3 py-2 rounded-lg border border-border text-sm">
              <option value="students">Academy Students</option>
              <option value="subscribers">Website Subscribers</option>
            </select>
          </div>

          {targetType === 'students' && (
            <div>
              <label className="block text-xs font-bold text-ink-muted uppercase mb-1">Send To (School)</label>
              <select value={schoolSlug} onChange={e => setSchoolSlug(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-border text-sm">
          ...
          <div>
            ...
            <select value={sender} onChange={e => setSender(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-border text-sm">
          ...
          <div>
            ...
            <input type="text" value={subject} onChange={e => setSubject(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-border text-sm" placeholder="Email subject" />
          </div>

          <div>
            <label className="block text-xs font-bold text-ink-muted uppercase mb-1">Message</label>
            <textarea value={message} onChange={e => setMessage(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-border text-sm" rows={10} placeholder="Write your message here. Use [Name] to personalize." />
            <p className="text-xs text-ink-muted mt-1">
              Recipients will receive: <strong>"Dear [Name], ..."</strong>
            </p>
          </div>
          ...
          <button onClick={handleSend} disabled={sending} className="px-6 py-3 bg-[#f97316] text-white font-bold rounded-xl hover:bg-[#ea6a05] transition-colors text-sm disabled:opacity-60">
